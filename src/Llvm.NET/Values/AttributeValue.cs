...
namespace Llvm.NET.Values
{
        ...
        : IEquatable<AttributeValue>
    {
        ...
        public string Name
        {
            get
            {
                if( IsString )
                {
                    return Marshal.PtrToStringAnsi( NativeMethods.GetStringAttributeKind( NativeAttribute, out uint length ) );
                }
                else
                {
        ...
        public string StringValue
        {
            get
            {
                ...
                return Marshal.PtrToStringAnsi( NativeMethods.GetStringAttributeValue( NativeAttribute, out uint len ) );
            }
        }
        ...
        {
            if(!( value.IsCallSite || value.IsFunction ) ) // TODO: Attributes on globals??
            {
        ...
        internal readonly LLVMAttributeRef NativeAttribute;
    }
    ...
    public static class AttributeValueMixins
    {
        ...
        public static AttributeValue ToAttributeValue( this AttributeKind kind, Context ctx ) => ctx.CreateAttribute( kind );

        /// <summary>Implicitly cast a string to an named <see cref="AttributeValue"/></summary>
        /// <param name="kind">Attribute name</param>
        /// <param name="ctx">Context that should own the attribute value</param>
        public static AttributeValue ToAttributeValue( this string kind, Context ctx ) => ctx.CreateAttribute( kind );
    }
}
